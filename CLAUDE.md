...
|--------|-------|----------|
| bridge.ts | 116 | 96% (Stmts/Branch/Funcs/Lines) |
| token.ts | 16 | — |
| screen.ts | 28 | 100% |
...
- WS Token Auth: per-session JWT HS256 token via `?token=` query param. Invalid tokens → close 4001. Token-less connections accepted with warning (backward compat)
